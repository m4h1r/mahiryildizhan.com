...
<div class="grid gap-4 md:grid-cols-2">
    ...
    <label class="md:col-span-2 text-sm font-medium text-gray-700 dark:text-gray-200">
        <span class="mb-1 block">Excerpt</span>
        <textarea name="excerpt" class="form-input-admin min-h-24" placeholder="Leave empty to auto-generate from body">{{ old('excerpt', $post->excerpt ?? '') }}</textarea>
    </label>

    <label class="md:col-span-2 text-sm font-medium text-gray-700 dark:text-gray-200">
        <span class="mb-1 block">Body</span>
        <div
        ...
        >
            <div class="flex flex-wrap items-center gap-2">
                <button type="button" @click="editor?.chain().focus().toggleBold().run()" class="rounded-md border border-gray-300 px-3 py-2 text-xs font-medium dark:border-gray-700">Bold</button>
                <button type="button" @click="editor?.chain().focus().toggleItalic().run()" class="rounded-md border border-gray-300 px-3 py-2 text-xs font-medium dark:border-gray-700">Italic</button>
                ...
                <button type="button" @click="editor?.chain().focus().toggleBulletList().run()" class="rounded-md border border-gray-300 px-3 py-2 text-xs font-medium dark:border-gray-700">List</button>
                <button type="button" @click="triggerImagePicker()" class="rounded-md border border-gray-300 px-3 py-2 text-xs font-medium dark:border-gray-700">Image</button>
                <button type="button" @click="openMediaLibrary()" class="rounded-md border border-gray-300 px-3 py-2 text-xs font-medium dark:border-gray-700">Media Library</button>
                <span x-show="loadingImage" x-cloak class="text-xs text-gray-500">Uploading image...</span>
            </div>
            ...
            <p class="text-xs text-gray-500">The textarea remains as a no-JS fallback and mirrors the editor HTML content.</p>
            ...
            <div x-show="showMediaLibrary" x-cloak class="rounded-lg border border-gray-300 bg-white p-3 dark:border-gray-700 dark:bg-gray-900">
                <div class="mb-2 flex items-center justify-between">
                    <p class="text-xs font-semibold">Select from media library</p>
                    <button type="button" @click="showMediaLibrary = false" class="text-xs text-gray-500">Close</button>
                </div>
                <div class="mb-3 grid gap-2 sm:grid-cols-[1fr_auto_auto]">
                    ...
                    <button type="button" @click="fetchMediaLibrary(1)" class="rounded-md border border-gray-300 px-3 py-2 text-xs font-medium dark:border-gray-700">Refresh</button>
                </div>
                <p x-show="loadingLibrary" class="text-xs text-gray-500">Loading media...</p>
                ...
                <div class="mt-3 flex items-center justify-between text-xs text-gray-500" x-show="libraryMeta.last_page > 1">
                    <button type="button" @click="fetchMediaLibrary(libraryMeta.page - 1)" :disabled="libraryMeta.page <= 1" class="rounded-md border border-gray-300 px-2 py-1 disabled:opacity-50 dark:border-gray-700">Prev</button>
                    <span x-text="`Page ${libraryMeta.page} / ${libraryMeta.last_page}`"></span>
                    <button type="button" @click="fetchMediaLibrary(libraryMeta.page + 1)" :disabled="libraryMeta.page >= libraryMeta.last_page" class="rounded-md border border-gray-300 px-2 py-1 disabled:opacity-50 dark:border-gray-700">Next</button>
                </div>
            </div>
        </div>
    ...
    <label class="md:col-span-2 text-sm font-medium text-gray-700 dark:text-gray-200">
        <span class="mb-1 block">Cover Image</span>
        <input type="file" name="cover_upload" accept="image/*" class="form-input-admin">
        <span class="mt-2 block text-xs text-gray-500">or choose from media library:</span>
        ...
        @if (! empty($post->cover ?? null))
            <span class="mt-2 block text-xs text-gray-500">Current cover: {{ $post->cover }}</span>
        @endif
    ...
    <label class="text-sm font-medium text-gray-700 dark:text-gray-200">
        <span class="mb-1 block">Publish Date</span>
